...
func NewService(r userRepo, j jwtSigner) *Service { return &Service{
		repo: r,
		jwt:  j,
...
func (s *Service) RefreshHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(r.Body).Decode(&in); err != nil || in.Refresh == "" {
		httpError(w, 400, "invalid_request"); return
	}

	// проверка blacklist
	if exp, ok := s.refreshBlacklist[in.Refresh]; ok && exp > time.Now().Unix() {
		httpError(w, 401, "token_revoked"); return
	}

	claims, err := s.jwt.Parse(in.Refresh)
	if err != nil { httpError(w, 401, "invalid_token"); return }
...
func (s *Service) GetUserHandler(w http.ResponseWriter, r *http.Request) {
	...
	u, ok := users[id]
	if !ok { httpError(w, 404, "not_found"); return }
