...
// ListDone — возвращает задачи с нужным значением done (true = выполненные, false = невыполненные)
...
// CreateMany — массовая вставка задач через транзакцию
func (r *Repo) CreateMany(ctx context.Context, titles []string) error {
	...
	for _, title := range titles {
		...
		if _, err = tx.ExecContext(ctx, q, title, done); err != nil {
			return err
		}
	}
